...
class Sale extends Model 
{
    ...
    private static $translates_class = 'App\Models\SalesTranslate';
    private static $main_table = 'sales';
    private static $translates_table = 'sales_translates';
    ...
    public static function addItem($request) 
    {
        ...
        $request_keys = $request->except(['_token','translates','image','status','show_on_index']);
        
        $item->status = $request->status === 'on' ? 1 : 0;
        $item->show_on_index = $request->show_on_index === 'on' ? 1 : 0;
    ...
    public static function getItemInfo($id = 0, $local = '') 
    {
        if(property_exists(__CLASS__, 'translates_table'))
        {
            return self::$current_class::join(self::$translates_table, self::$main_table.'.id', '=', self::$translates_table.'.parent_id')
                ->join('products', self::$main_table.'.product_id', '=', 'products.id')
